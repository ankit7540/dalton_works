...
refg=(0.2088867 0.4241638 0.7504779 1.2051277 1.8087614 2.5586830 3.4528310 4.4597657 5.5327314 6.6057220 7.5932115 8.3989993 8.9415832 9.1647024 9.0545598 8.7203779 7.9847883 7.1732613 5.0021597 3.2164592 1.2703487 0.5569574 0.2924626 0.1830105 0.1259522)
...
basis=$(grep -n "X2          2    0.0000" $fname$i.out)
...
posn=$(grep 'X      0.00000      0.00000'  $fname$i.out  | awk '{print $4}'|xargs printf "%2.2f\n" | xargs)
...
echo -e $i,$posn,$posn2 ,$xxn,${refxa[$i-1]} ,$delta_x,$zzn,$refzn,$delta_z,"processed"
...
printf "%g\t%s\t%f (%f)\t%g\t%2.3f\t%2.3f-%2.3f\t%4.7f\t%4.7f\t%4.7f\t%4.7f\t%4.7f\t%4.7f\t%s\n" "$i" "$bs" "$exp1" "$step"   "$rn" "$bd" "$posn" "$posn2"  "$xxn" "$delta_x" "$zzn" "$delta_z" "$delta_mp" "$delta_g" "$t1" >>  result_$folder.txt
