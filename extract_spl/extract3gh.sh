...
basis=$(grep -n "X3          3    0.0000" $fname$i.out)
...
printf "%g\t%s\t%f (%f)\t%g\t%2.3f\t%2.3f-%2.3f-%2.3f\t%4.7f\t%4.7f\t%4.7f\t%4.7f\t%4.7f\t%4.7f\t%s\n" "$i" "$bs" "$exp1" "$step"   "$rn" "$bd" "$posn" "$posn2" "$posn3"  "$xxn" "$delta_x" "$zzn" "$delta_z" "$delta_mp" "$delta_g" "$t1" >>  result_$folder.txt
